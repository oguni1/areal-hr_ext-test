...
const File = {
    ...
    async delete(id) {
        const { rows } = await pool.query('DELETE FROM files WHERE id = $1 RETURNING *', [id])
        return rows[0]
    }
}
...
const Passport = {
    ...
    async getById(id) {
        const { rows } = await pool.query('SELECT * FROM passports WHERE id = $1', [id])
        return rows[0]
    },
    ...
    async delete(id) {
        const { rows } = await pool.query('DELETE FROM passports WHERE id = $1 RETURNING *', [id])
        return rows[0]
    }
}
...
const Registration = {
    ...
    async delete(id) {
        const { rows } = await pool.query('DELETE FROM registrations WHERE id = $1 RETURNING *', [id])
        return rows[0]
    }
}
...
const OperationHistory = {
    getAll: async () => {
        const { rows } = await pool.query('SELECT * FROM operation_history ORDER BY id DESC')
        return rows
    },
    getById: async (id) => {
        const { rows } = await pool.query('SELECT * FROM operation_history WHERE id = $1', [id])
        return rows[0]
    },
    create: async (data) => {
        const { rows } = await pool.query(
            'INSERT INTO operation_history (changed_by, object_entity, obj_entity_id, changed_fields, type) VALUES ($1, $2, $3, $4, $5) RETURNING *',
            [data.changed_by, data.object_entity, data.obj_entity_id, data.changed_fields, data.type]
        )
        return rows[0]
    },
    update: async (id, data) => {
        const { rows } = await pool.query(
            ...
            [data.changed_by, data.object_entity, data.obj_entity_id, data.changed_fields, data.type, id]
        )
        return rows[0]
    },
    delete: async (id) => {
        const { rows } = await pool.query('DELETE FROM operation_history WHERE id = $1 RETURNING *', [id])
        return rows[0]
    }
}
...
class Users {
    ...
    async create(user) {
        const { rows } = await pool.query(
            ...
            [user.surname, user.name, user.patronym, user.login, user.password, user.role]
        )
        return rows[0]
    }
    ...
    async update(id, user) {
        const { rows } = await pool.query(
            ...
            [user.surname, user.name, user.patronym, user.login, user.password, user.role, id]
        )
        return rows[0]
    }
...
module.exports = { Organization, Role, Department, Employee, File, Passport, Registration, OperationHistory, Users };