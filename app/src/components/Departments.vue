    ...
    <div>
      <h2>Список отделов</h2>
        <div v-for="department in departments" :key="department.id">
            ...
            <template v-if="editingDepartment && editingDepartment.id === department.id">
              ...
              <input v-model="editingDepartment.name" placeholder="Название отдела">
              <input v-model="editingDepartment.comment" placeholder="Комментарий">
              <button @click="updateDepartment">Сохранить</button>
              <button @click="cancelEdit">Отмена</button>
            </template>
            <template v-else>
              <p>Организация: {{ getOrganizationName(department.organization_id) }}</p>
              <p>Родительский отдел: {{ getDepartmentName(department.department_id) }}</p>
...
export default {
  ...
  methods: {
    getNextId() {
      ...
      const maxId = Math.max(...this.departments.map(dept => parseInt(dept.id)))
      return maxId + 1
    },
    ...
    async updateDepartment() {
      if (!this.editingDepartment.name || !this.editingDepartment.organization_id) {
        alert('Название отдела и организация обязательны!')
        return
      }

      try {
        await this.$api.put(`/departments/${this.editingDepartment.id}`, this.editingDepartment)
        this.editingDepartment = null
        await this.fetchDepartments()
      } catch (error) {
