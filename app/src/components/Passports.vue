<template>
  <div class="container">
    ...
    <div>
      ...
      <input
        v-model.number="newPassport.code"
        type="number"
    ...
    <div>
      ...
      <div v-for="passport in passports" :key="passport.id">
        ...
        <template v-if="editingPassport && editingPassport.id === passport.id">
          ...
          <input
            v-model="editingPassport.date"
            type="date"
            placeholder="Дата выдачи"
          />
          <input
            v-model.number="editingPassport.code"
            type="number"
            placeholder="Код подразделения"
          />
  ...
  methods: {
    getNextId() {
      ...
      const maxId = Math.max(
        ...this.passports.map((pass) => parseInt(pass.id))
      );
      return maxId + 1;
    },
    ...
    async addPassport() {
      if (
        !this.newPassport.series ||
        !this.newPassport.number ||
        !this.newPassport.date ||
        !this.newPassport.code ||
        !this.newPassport.issued_by
      ) {
    ...
    async updatePassport() {
      if (
        !this.editingPassport.series ||
        !this.editingPassport.number ||
        !this.editingPassport.date ||
        !this.editingPassport.code ||
        !this.editingPassport.issued_by
      ) {
        alert('Все поля обязательны!');
        return;
      }
      ...
      try {
        ...
        await this.$api.put(
          `/passports/${this.editingPassport.id}`,
          updateData
        );
        this.editingPassport = null;
        await this.fetchPassports();
