<template>
  <div class="container">
    ...
    <div>
      ...
      <input
        v-model="newHistory.changed_by"
        type="number"
        placeholder="ID пользователя"
      />
      <input v-model="newHistory.object_entity" placeholder="Сущность" />
      <input
        v-model.number="newHistory.obj_entity_id"
        type="number"
        placeholder="ID сущности"
      />
      <input
        v-model="newHistory.changed_fields"
        placeholder="Измененные поля"
      />
    ...
    <div>
      ...
      <div v-for="history in histories" :key="history.id">
        ...
        <template v-if="editingHistory && editingHistory.id === history.id">
          <input
            v-model.number="editingHistory.changed_by"
            type="number"
            placeholder="ID пользователя"
          />
          <input
            v-model="editingHistory.object_entity"
            placeholder="Сущность"
          />
          <input
            v-model.number="editingHistory.obj_entity_id"
            type="number"
            placeholder="ID сущности"
          />
          <input
            v-model="editingHistory.changed_fields"
            placeholder="Измененные поля"
          />
...
export default {
  ...
  methods: {
    ...
    async addHistory() {
      if (
        !this.newHistory.changed_by ||
        !this.newHistory.object_entity ||
        !this.newHistory.obj_entity_id ||
        !this.newHistory.type
      ) {
    ...
    async updateHistory() {
      if (
        !this.editingHistory.changed_by ||
        !this.editingHistory.object_entity ||
        !this.editingHistory.obj_entity_id ||
        !this.editingHistory.type
      ) {
        alert('Все поля обязательны для заполнения!');
        return;
      }
      ...
      try {
        ...
        await this.$api.put(
          `/operation-history/${this.editingHistory.id}`,
          updateData
        );
        this.editingHistory = null;
        await this.fetchHistories();
